...
@register()
def memberaudit_config_check(app_configs, **kwargs):  # pylint: disable=W0613
    """memberaudit_config_check is a Django check that verifies that all periodic tasks
    ...
    for name, obj in settings.CELERYBEAT_SCHEDULE.items():
        _verify_task_config(
            warnings, name, obj, "memberaudit.tasks.run_regular_updates"
        )
    return warnings
